...
int encx264Init( hb_work_object_t * w, hb_job_t * job )
{
    ...
    if (job->color_transfer == HB_COLR_TRA_SMPTEST2084)
    {
        ...
        if (job->mastering.has_primaries && job->mastering.has_luminance)
        {
            ...
            param.mastering_display.i_red_x   = hb_rescale(job->mastering.display_primaries[0][0], MASTERING_CHROMA_DEN);
            param.mastering_display.i_red_y   = hb_rescale(job->mastering.display_primaries[0][1], MASTERING_CHROMA_DEN);
            param.mastering_display.i_green_x = hb_rescale(job->mastering.display_primaries[1][0], MASTERING_CHROMA_DEN);
            param.mastering_display.i_green_y = hb_rescale(job->mastering.display_primaries[1][1], MASTERING_CHROMA_DEN);
            param.mastering_display.i_blue_x  = hb_rescale(job->mastering.display_primaries[2][0], MASTERING_CHROMA_DEN);
            param.mastering_display.i_blue_y  = hb_rescale(job->mastering.display_primaries[2][1], MASTERING_CHROMA_DEN);
            param.mastering_display.i_white_x = hb_rescale(job->mastering.white_point[0], MASTERING_CHROMA_DEN);
            param.mastering_display.i_white_y = hb_rescale(job->mastering.white_point[1], MASTERING_CHROMA_DEN);
            param.mastering_display.i_display_max = hb_rescale(job->mastering.max_luminance, MASTERING_LUMA_DEN);
            param.mastering_display.i_display_min = hb_rescale(job->mastering.min_luminance, MASTERING_LUMA_DEN);
        }
