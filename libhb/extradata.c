...
int hb_set_extradata(hb_buffer_t **extradata, const uint8_t *bytes, size_t length)
{
    hb_buffer_close(extradata);

    if (length > 0)
    {
        *extradata = hb_buffer_init(length);
        if (*extradata == NULL)
        {
            hb_error("extradata: malloc failure");
            return 1;
        }
        memcpy((*extradata)->data, bytes, length);
    }
    return 0;
}

int hb_set_text_extradata(hb_buffer_t **extradata, const uint8_t *bytes, size_t length)
{
    hb_buffer_close(extradata);

    if (length > 0)
    {
        *extradata = hb_buffer_init(length + 1);
        if (*extradata == NULL)
        {
            hb_error("extradata: malloc failure");
            return 1;
...
int hb_set_ssa_extradata(hb_buffer_t **extradata, const char *font, int fs, int w, int h)
{
    hb_buffer_close(extradata);
    ...
    *extradata = hb_buffer_init(size);
    if (*extradata == NULL)
    {
        goto fail;
    }

    memcpy((*extradata)->data, header, size);
    free(header);
...
int hb_set_h264_extradata(hb_buffer_t **extradata, uint8_t *sps, size_t sps_length, uint8_t *pps, size_t pps_length)
{
    hb_buffer_close(extradata);
...
int hb_set_xiph_extradata(hb_buffer_t **extradata, uint8_t headers[3][HB_CONFIG_MAX_SIZE])
{
    hb_buffer_close(extradata);
