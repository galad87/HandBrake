...
static int set_extradata(hb_buffer_t *extradata, uint8_t **priv_data, int *priv_size)
{
    if (*priv_data)
    {
        av_freep(priv_data);
    ...
    if (extradata && extradata->size > 0)
    {
        ...
        *priv_data = av_malloc(extradata->size + AV_INPUT_BUFFER_PADDING_SIZE);
        ...
        memcpy(*priv_data, extradata->data, extradata->size);
    }
    return 0;
}
        ...
        switch (subtitle->source)
        {
            ...
            case DVBSUB:
            {
                ...
                if (subtitle->extradata != NULL && subtitle->extradata->size)
                {
                    ...
                    priv_data = av_malloc(priv_size + AV_INPUT_BUFFER_PADDING_SIZE);
                    memcpy(priv_data, subtitle->extradata->data, priv_size);
                }
            } break;
