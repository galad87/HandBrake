...
int encx265Init(hb_work_object_t *w, hb_job_t *job)
{
    ...
    if (job->color_transfer == HB_COLR_TRA_SMPTEST2084)
    {
        ...
        if (job->mastering.has_primaries && job->mastering.has_luminance)
        {
            ...
            snprintf(masteringDisplayColorVolume, sizeof(masteringDisplayColorVolume),
                     ...
                     (unsigned short)hb_rescale(job->mastering.display_primaries[1][0], MASTERING_CHROMA_DEN),
                     (unsigned short)hb_rescale(job->mastering.display_primaries[1][1], MASTERING_CHROMA_DEN),
                     (unsigned short)hb_rescale(job->mastering.display_primaries[2][0], MASTERING_CHROMA_DEN),
                     (unsigned short)hb_rescale(job->mastering.display_primaries[2][1], MASTERING_CHROMA_DEN),
                     (unsigned short)hb_rescale(job->mastering.display_primaries[0][0], MASTERING_CHROMA_DEN),
                     (unsigned short)hb_rescale(job->mastering.display_primaries[0][1], MASTERING_CHROMA_DEN),
                     (unsigned short)hb_rescale(job->mastering.white_point[0], MASTERING_CHROMA_DEN),
                     (unsigned short)hb_rescale(job->mastering.white_point[1], MASTERING_CHROMA_DEN),
                     (unsigned)hb_rescale(job->mastering.max_luminance, MASTERING_LUMA_DEN),
                     (unsigned)hb_rescale(job->mastering.min_luminance, MASTERING_LUMA_DEN));
    ...
    {
        param->ambientIlluminance = hb_rescale(job->ambient.ambient_illuminance, 10000);
        param->ambientLightX = hb_rescale(job->ambient.ambient_light_x, 50000);
        param->ambientLightY = hb_rescale(job->ambient.ambient_light_y, 50000);
